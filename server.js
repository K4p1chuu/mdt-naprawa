...
// Dodatkowe middleware dla preflight requests
app.options('*', cors(corsOptions));

// Dodatkowe nagłówki bezpieczeństwa (na wypadek problemów)
...
app.post('/api/leaves', verifyToken, async (req, res) => {
    try {
        ...
        if (!reason || !dateRange || !canExtend || !signature) {
        ...
        // Wyślij webhook na Discord (jeśli skonfigurowany)
        if (UERD_LEAVE_WEBHOOK_URL && !UERD_LEAVE_WEBHOOK_URL.includes('placeholder')) {
            try {
                await fetch(UERD_LEAVE_WEBHOOK_URL, {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        embeds: [{
                            title: '🏖️ Nowy wniosek urlopowy',
                            color: 0x00ff00,
                            fields: [
                                { name: 'Funkcjonariusz', value: `[${newLeave.officerBadge}] ${newLeave.officerName}`, inline: true },
                                { name: 'Ranga', value: newLeave.officerRank, inline: true },
                                { name: 'Departament', value: newLeave.department, inline: true },
                                { name: 'Powód', value: reason },
                                { name: 'Okres', value: dateRange },
                                { name: 'Możliwość przedłużenia', value: canExtend },
                                { name: 'Podpis', value: signature }
                            ],
                            timestamp: new Date().toISOString()
                        }]
                    })
                ...
                console.error('[WEBHOOK] Błąd wysyłania webhooka urlopowego:', webhookError);
            }
        }
        
        res.json({ success: true, leave: newLeave });
    } catch (error) {
        console.error('[API] Błąd dodawania urlopu:', error);
...
app.delete('/api/leaves/:id', verifyToken, (req, res) => {
    try {
        ...
        if (leaveIndex === -1) {
            return res.status(404).json({ success: false, message: 'Nie znaleziono urlopu' });
        }
        
        const leave = leaves[leaveIndex];
        
        // Sprawdź uprawnienia - admin lub właściciel urlopu
        if (!req.user.isAdmin && leave.officerBadge !== req.user.badge) {
            return res.status(403).json({ success: false, message: 'Brak uprawnień do zakończenia tego urlopu' });
        }
        
        leaves.splice(leaveIndex, 1);
        saveData(DB.leaves, leaves);
        
        res.json({ success: true, message: 'Urlop został zakończony' });
    } catch (error) {
        console.error('[API] Błąd usuwania urlopu:', error);
        res.status(500).json({ success: false, message: 'Błąd serwera', error: error.message });
    }
});
    ...
    const officersWithStats = Object.values(users)
        ...
        .map(user => {
            ...
            return {
                ...
                onLeave: leaves.some(leave => leave.officerBadge === user.badge || leave.officerName === user.username),
...
// POST /api/officers - Dodaj nowego funkcjonariusza
app.post('/api/officers', verifyToken, verifyAdmin, (req, res) => {
    try {
        ...
        console.log('[DEBUG] POST /api/officers - dane:', req.body);
        ...
        console.log('[API] POST /api/officers - dodano nowego użytkownika:', username);
        res.json({ success: true, user: newUser });
    } catch (error) {
        console.error('[API] Błąd dodawania funkcjonariusza:', error);
        res.status(500).json({ success: false, message: 'Błąd serwera', error: error.message });
    }
});
...
app.put('/api/officers/:email', verifyToken, verifyAdmin, (req, res) => {
    try {
        const email = decodeURIComponent(req.params.email);
        console.log('[DEBUG] PUT /api/officers/:email - email:', email);
        console.log('[DEBUG] PUT /api/officers/:email - body:', req.body);
        
        if (!users[email]) {
            return res.status(404).json({ success: false, message: 'Nie znaleziono oficera' });
        }
        
        const { username, badge, rank, department, password, robloxId, discordNick, robloxNick, joinedAt, newEmail } = req.body;
        
        // Jeśli zmienia się email, sprawdź czy nowy email nie jest już zajęty
        if (newEmail && newEmail !== email) {
            if (users[newEmail]) {
                return res.status(400).json({ success: false, message: 'Użytkownik o tym emailu już istnieje' });
            }
            
            // Przenieś użytkownika na nowy email
            users[newEmail] = { ...users[email] };
            users[newEmail].email = newEmail;
            delete users[email];
            
            // Aktualizuj inne dane na nowym emailu
            if (username) users[newEmail].username = username;
            if (badge) users[newEmail].badge = badge;
            if (rank) users[newEmail].rank = rank;
            if (department) users[newEmail].department = department;
            if (password) users[newEmail].password = password;
            if (robloxId !== undefined) users[newEmail].robloxId = robloxId;
            if (discordNick !== undefined) users[newEmail].discordNick = discordNick;
            if (robloxNick !== undefined) users[newEmail].robloxNick = robloxNick;
            if (joinedAt !== undefined) users[newEmail].joinedAt = joinedAt;
            
            saveData(DB.users, users);
            console.log('[API] PUT /api/officers/:email - zmieniono email z', email, 'na', newEmail);
            return res.json({ success: true, officer: users[newEmail] });
        }
        
        // Jeśli email się nie zmienia, aktualizuj tylko dane
        ...
        if (robloxId !== undefined) users[email].robloxId = robloxId;
        if (discordNick !== undefined) users[email].discordNick = discordNick;
        if (robloxNick !== undefined) users[email].robloxNick = robloxNick;
        if (joinedAt !== undefined) users[email].joinedAt = joinedAt;
        
        saveData(DB.users, users);
        console.log('[API] PUT /api/officers/:email - zaktualizowano dane dla:', email);
        res.json({ success: true, officer: users[email] });
    } catch (error) {
        console.error('[API] Błąd edycji funkcjonariusza:', error);
        res.status(500).json({ success: false, message: 'Błąd serwera', error: error.message });
    }
});
